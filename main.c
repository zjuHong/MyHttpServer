...
void sig_handler(int sig)
{
    ...
    int msg = sig;
    send(pipefd[1], (char *)&msg, 1, 0);//写管道触发epoll
    errno = save_errno;
}
...
void addfd_(int epollfd, int fd, bool one_shot)
{
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLRDHUP; //输入/断开
    if (one_shot)
...
int main(int argc, char *argv[])
{
    ...
    {
        port = 2019;
        printf("using default port:2019\n");
    }
    else
        port = atoi(argv[1]);
    ...
    //单例模式创建数据库连接池 mysql_real_connect(mysql,"localhost","root",NULL,"test",0,NULL,0);
    ...
    addsig(SIGTERM, sig_handler, false); //kill触发信号
    bool stop_server = false;

    client_data *users_timer = new client_data[MAX_FD];//定时器的map，到fd、address的映射
    bool timeout = false;
    alarm(TIMESLOT);
    ...
    while (!stop_server)
    {
        ...
        for (int i = 0; i < number; i++)
        {
            ...
            if (sockfd == listenfd)//处理新到的客户连接
            {
                ...
                users[connfd].init(connfd, client_address);//每次接收都做初始化了
                //初始化client_data数据
            ...
            // 后两个事件应该是本端（server端）出错才触发的。
            // EPOLLRDHUP表示读关闭，EPOLLHUP是读写都关闭
            ...
            {
                util_timer *timer = users_timer[sockfd].timer;//时间定时器是关于fd的map
            ...
            else if (events[i].events & EPOLLOUT)//读信号
            {
               util_timer *timer = users_timer[sockfd].timer;
